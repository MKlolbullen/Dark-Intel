...
Two scraper-quality knobs learned from live runs: the `news` scraper now sends a real Chrome User-Agent so Reuters / other news sites stop returning 401, and when the user pastes a `Your website` URL the news and competitor scrapers both **discover real navigation paths dynamically** instead of guessing `/about`, `/pricing`, etc. They fetch the homepage, parse its `<a>` hrefs, and keep internal paths whose last slug is in `app/scrapers/_html.py:NAV_KEYWORDS` — covers English plus Swedish, German, French, Spanish, Italian, and Dutch equivalents (om-oss, uber-uns, a-propos, sobre, chi-siamo, over-ons, …). That fixed the `ica.se` case where every English path 404'd.

When a page's main body collapses to under 200 chars (typical for JS-only SPA shells), both scrapers fall back to `extract_meta_summary` — title + `og:description` / meta description / Twitter card — so even a pure client-rendered site contributes at least its positioning blurb instead of being silently dropped. Server logs at INFO tell you when discovery found N paths, when a page's body was thin and meta was used, and when a page was dropped entirely. Hacker News results are also filtered client-side by a word-boundary regex on the business name — look for `"hn dropped N irrelevant hits"` to see the savings.
